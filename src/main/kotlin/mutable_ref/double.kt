    ...
    private fun mainLoop(g: Graphics2D) {
        repeat(100) {
            ...
            for (x in 0 until screenWidth) {
                ...
                val rayDir = dir.plus(plane.times(cameraX, wrapper).decodeVector2d(wrapper), wrapper).decodeVector2d(wrapper)
                ...
                if (rayDir.x < 0) {
                    ...
                    sideDist = (pos.toVector(wrapper).decodeVector2d(wrapper).minus(mapLocation.toVector(wrapper).decodeVector2d(wrapper), wrapper).decodeVector2d(wrapper)).xProjection(wrapper).decodeVector2d(wrapper)
                        .times(deltaDist.x, wrapper).decodeVector2d(wrapper)
                } else {
                    step = Vector2d(1.toDouble(), 0.0)
                    sideDist = (mapLocation.toVector(wrapper).decodeVector2d(wrapper).plus(Vector2d(1.0, 0.0), wrapper).decodeVector2d(wrapper)
                        .minus(pos.toVector(wrapper).decodeVector2d(wrapper), wrapper).decodeVector2d(wrapper)).xProjection(wrapper).decodeVector2d(wrapper)
                        .times(deltaDist.x, wrapper).decodeVector2d(wrapper)
                }
                if (rayDir.y < 0) {
                    step = step.plus(Vector2d(0.0, (-1).toDouble()), wrapper).decodeVector2d(wrapper)
                    sideDist = sideDist.plus(
                        (pos.toVector(wrapper).decodeVector2d(wrapper).minus(mapLocation.toVector(wrapper).decodeVector2d(wrapper), wrapper).decodeVector2d(wrapper)).yProjection(wrapper).decodeVector2d(wrapper)
                            .times(deltaDist.y, wrapper).decodeVector2d(wrapper), wrapper
                    ).decodeVector2d(wrapper)
                } else {
                    step = step.plus(Vector2d(0.0, 1.toDouble()), wrapper).decodeVector2d(wrapper)
                    sideDist = sideDist.plus(
                        (mapLocation.toVector(wrapper).decodeVector2d(wrapper).plus(Vector2d(0.0, 1.0), wrapper).decodeVector2d(wrapper)
                            .minus(pos.toVector(wrapper).decodeVector2d(wrapper), wrapper).decodeVector2d(wrapper)).yProjection(wrapper).decodeVector2d(wrapper)
                            .times(deltaDist.y, wrapper).decodeVector2d(wrapper), wrapper
                    ).decodeVector2d(wrapper)
                }
                //perform DDA
                while (hit == 0) {
                    ...
                    if (sideDist.x < sideDist.y) {
                        sideDist = sideDist.plus(deltaDist.xProjection(wrapper).decodeVector2d(wrapper), wrapper).decodeVector2d(wrapper)
                        mapLocation = mapLocation.step(step.xProjection(wrapper).decodeVector2d(wrapper), wrapper).decodeLocationD(wrapper)
                        side = 0
                    } else {
                        sideDist = sideDist.plus(deltaDist.yProjection(wrapper).decodeVector2d(wrapper), wrapper).decodeVector2d(wrapper)
                        mapLocation = mapLocation.step(step.yProjection(wrapper).decodeVector2d(wrapper), wrapper).decodeLocationD(wrapper)
                        side = 1
                    }
