...
inline fun returnPoint2d() = Point2d(decodeDouble0(), decodeDouble1())
context (MutableMfvcWrapper)
inline fun returnVector2d() = Vector2d(decodeDouble0(), decodeDouble1())
context (MutableMfvcWrapper)
inline fun returnLocationD() = LocationD(decodeInt0(), decodeInt1())
...
value class LocationD(val x: Int, val y: Int) {
    context(MutableMfvcWrapper)
    inline fun toVector(): Vector2d  {
        encodeLong0(x.toDouble())
        encodeLong1(y.toDouble())
        return returnVector2d()
    }
            ...
            with (MutableMfvcWrapper()) {
                for (x in 0 until screenWidth) {
                    ...
                    if (rayDir.x < 0) {
                        ...
                        step = Vector2d(1.toDouble(), 0.0)
                        sideDist =
                            (mapLocation.toVector() + Vector2d(1.0, 0.0) - pos.toVector()).xProjection() * deltaDist.x
                    }
                    if (rayDir.y < 0) {
                        ...
                        step += Vector2d(0.0, 1.toDouble())
                        sideDist += (mapLocation.toVector() + Vector2d(
                            0.0,
                            1.0
